...
minFuncDir = ['tools', filesep, 'minFunc_2012', filesep];

saveDir = ['models',filesep,'training'];

%% add paths
addpath(['tools', filesep,'labrosa']);
addpath(['tools', filesep,'bss_eval']);
...
CFGPath=['tools',filesep,'htk_features', filesep];
addpath(CFGPath);
...
modelname=[modelname,'_cutSilence',num2str(eI.cut_silences)]
...
eI.saveDir = [saveDir, filesep, runID ,'__',num2str(round(abs(randn(1,1)*1000))),'__',modelname, filesep];
...
eI.DataPath=train_data_path;
eI.ValDataPath=val_data_path;

%eI.DataPath=[codeDir,'mir1k', filesep, 'Wavfile',filesep];
%train_files= dir( [eI.DataPath, 'train',filesep,'*wav']);

% chunk
[data_cell, targets_cell, mixture_spectrum]=formulate_data(eI.train_files, eI, eI.train_mode); %0 -- chunk, 2--no chunk, 3- icassp
